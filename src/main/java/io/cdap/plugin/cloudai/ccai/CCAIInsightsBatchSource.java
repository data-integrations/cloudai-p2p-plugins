...
public class CCAIInsightsBatchSource extends BatchSource<LongWritable, StructuredRecord, StructuredRecord> {
  private static final Logger LOG = LoggerFactory.getLogger(CCAIInsightsBatchSource.class);

  private final io.cdap.plugin.cloudai.ccai.CCAIInsightsBatchSourceConfig config;

  public CCAIInsightsBatchSource(
      io.cdap.plugin.cloudai.ccai.CCAIInsightsBatchSourceConfig config) {
    this.config = config;
  }
  ...
  public void prepareRun(BatchSourceContext batchSourceContext) throws Exception {
    ...
    batchSourceContext.setInput(
        Input.of(config.getReferenceName(), new SourceInputFormatProvider(
            io.cdap.plugin.cloudai.ccai.CCAIInsightsInputFormat.class, config.getConfiguration())));
  }
}
