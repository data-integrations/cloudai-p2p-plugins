...
public final class CCAIInsightsRecordReader extends RecordReader<Object, StructuredRecord> {
  private static final Logger LOG = LoggerFactory.getLogger(
      io.cdap.plugin.cloudai.ccai.CCAIInsightsRecordReader.class);
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext taskAttemptContext)
    ...
    String projectId = conf.get(CCAIInsightsBatchSourceConfig.NAME_PROJECT);
    ...
    iter = Futures.getUnchecked(conversationFuture).getConversationsList().parallelStream().map(
        conversation -> {
          ...
          if (tokens.length > 5) {
            String conversationId = tokens[5];
            return io.cdap.plugin.cloudai.ccai.InsightsModel
                .createExportRecord(conversationId, conversation, conversation.getLatestAnalysis());
          } else {
            LOG.error("Invalid analysis name {}", name);
          }
          return null;
